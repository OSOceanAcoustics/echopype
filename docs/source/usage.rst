...
.. warning::
   Previous versions of echopype used ``raw2nc`` and ``raw2zarr`` in order
   to convert to NetCDF or Zarr files respectively. These functions have
   now been renamed ``to_netcdf`` and ``to_zarr``.
...
Before calling ``to_netcdf()`` to create netCDF4 files,
...
.. note::
   ...
   The ``combine_opt`` attribute used in the old ``raw2nc()`` function
   has been renamed ``combine`` in the updated ``to_netcdf`` function.
...
- EK80 and EA640 broadband echosounder:
...
Additional information will be added for broadband EK80 and EA640 echosounder as
