...
def check_source_channel_order(ds_in: xr.Dataset, freq_ref: xr.DataArray) -> xr.Dataset:
    ...
    freq_ref : xr.DataArray
        An xr.DataArray containing the nominal frequency (with ``channel`` as a coordinate)
        in the order to be conformed with.
    ...
    """
    if not isinstance(freq_ref, xr.DataArray) or "channel" not in freq_ref.coords:
        raise ValueError("'freq_ref' has to be an xr.DataArray with coordinate 'channel'!")
