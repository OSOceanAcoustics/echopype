...
def open_raw(
    ...
    """Create an EchoData object from instrument-generated data.

    For BI500, this function returns two objects:

    - an EchoData object containing metadata and ancillary information;
    - an xarray Dataset containing the calibrated BI500 products.

    Parameters
    ----------
    raw_file : str or pathlib.Path
        Path to the raw data file or, for BI500, the directory containing
    ...
    sonar_model : str
        ...
        - ``AZFP6``: ASL Environmental Sciences AZFP6 echosounder
        - ``AD2CP``: Nortek Signature series ADCP
        - ``BI500``: Bergen Integrator BI500 data
    xml_path : str, optional
        Path to the XML configuration file used by AZFP.
    include_bot : bool, default False
        Include bottom-depth files where supported.
    include_idx : bool, default False
        Include index files where supported.
    convert_params : dict, optional
        Additional metadata parameters to include in the converted data.
    storage_options : dict, optional
        Options passed to the filesystem implementation.
    use_swap : bool or {"auto"}, default False
        Whether to use temporary disk storage during conversion.
    max_chunk_size : str, default "100MB"
        Maximum chunk size when temporary disk storage is used.
    ...
    tuple of EchoData and xarray.Dataset
        For BI500, returns ``(echodata, ds_cal)``, where ``echodata``
        contains metadata and ancillary BI500 information, and ``ds_cal``
        contains calibrated products generated by BI500, including
        ``Sv``, ``Sv_bottom``, ``echo_range``, ``echo_range_bottom``,
        and single-target variables.

    Raises
    ------
    ValueError
        If the sonar model is unsupported.
    FileNotFoundError
        If the input path does not exist.
    TypeError
        If the input path is not a string or pathlib.Path.
    """
    if raw_file is None:
    ...
    # Uppercase the sonar model in case the user passes lowercase text
    ...
    # Rectangularization and direct zarr offloading are currently
    # supported only for the Simrad Ex60 and Ex80 model families.
    ...
    # Allocate the Sonar group before adding its beam groups
    ...
    # Create the EchoData tree
    tree = DataTree.from_dict(tree_dict, name="root")

    echodata = EchoData(
        source_file=file_chk,
        xml_path=xml_chk,
        sonar_model=sonar_model,
    )
