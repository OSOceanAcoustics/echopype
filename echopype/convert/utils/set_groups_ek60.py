    ...
    def set_beam(self, beam_dict):
        ...
        if not os.path.exists(self.file_path):
            ...
            for i in list(range(pieces)):
                backscatter_r = np.array([beam_dict['power_dict'][i][x] for x in
                                         beam_dict['power_dict'][i].keys()])
                range_bin = beam_dict['range_bin'][i]
                ping_time = beam_dict['ping_time_split'][i]
                ds = xr.Dataset(
                    {'backscatter_r': (['frequency', 'ping_time', 'range_bin'], backscatter_r,
                ...
                if pieces == 1 or i == 0:
                    ...
                    if self.format == '.nc':
                        ds.to_netcdf(path=path, mode='w', group='Beam')
                    elif self.format == '.zarr':
                        ds.to_zarr(store=path, mode='w', group='Beam')
