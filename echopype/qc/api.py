...
def remove_duplicate_ping_times(echodata: EchoData):
    """
    Removes duplicate `ping_time` values from `ping_time` dimensions in an `EchoData` object.

    The duplicate `ping_time`s are located in the Provenance group in the `"duplicate_ping_times"`
    variable and are automatically generated during raw file conversion.

    Parameters
    ----------
    echodata: EchoData
        The `EchoData` object to remove the duplicate `ping_time` values from.

    Raises
    ------
    ValueError
        If the given `EchoData` does not contain duplicate `ping_time`s stored in the Provenance
        group in a `"duplicate_ping_times"` variable.
    """

    if "duplicate_ping_times" not in echodata.provenance:
        raise ValueError("EchoData object does not contain duplicate ping times")

    for group_name in EchoData.group_map:
        print(group_name)
        group = getattr(echodata, group_name)
        if "ping_time" in group:
            print(np.unique(echodata.provenance["duplicate_ping_times"].data.flatten()))
            setattr(
                echodata,
                group_name,
                group.drop_sel(
