...
def _create_np_array_from_echoview_mats(paths_to_echoview_mat: list):
    """
    ...
    Parameters
    ----------
    paths_to_echoview_mat: list of  TODO: fill in type
    ...
    -------
    np.ndarray
        A numpy array generated by choosing the appropriate data from the mat files
        and stacking them in the order determined by ``paths_to_echoview_mat``. This
        array will have dimensions ``(len(paths_to_echoview_mat), 2, range_sample)``
    """
    ...
    return np.stack(list_of_mat_arrays, axis=0)


@pytest.mark.parametrize(
    ("sonar_model", "test_path_key", "raw_file_name", "paths_to_echoview_mat", "waveform_mode", "encode_mode"),
    [
        (
...
def test_add_splitbeam_angle(sonar_model, test_path_key, raw_file_name, test_path,
    ...
    ds = ep.consolidate.add_splitbeam_angle(ds=ds_Sv, echodata=ed,
                                            waveform_mode=waveform_mode, encode_mode=encode_mode)

    # obtain corresponding echoview output
    echoview_mat = _create_np_array_from_echoview_mats(paths_to_echoview_mat)
