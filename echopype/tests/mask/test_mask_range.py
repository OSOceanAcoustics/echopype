...
def test_get_range_mask(
    n: int, n_chan_freq: int, channel: str, r0: float, r1: float, method: str, expected: np.ndarray
):
    def get_mock_range_mask_data(n: int, n_chan_freq: int) -> xr.Dataset:
        ...
        mock_Sv_data = [mat_A, np.identity(n), mat_B] + [
            np.identity(n) for i in range(3, n_chan_freq)
        ]
