...
def test_ocean_contour():
    for file, parsed_file in zip(AD2CP_UNPARSED, AD2CP_PARSED_BY_OCEAN_CONTOUR):
        ...
        assert np.isclose(base.attrs["Instrument_pressure"], test.attrs["pressure_sensor_valid"]).all()
        assert np.isclose(base.attrs["Instrument_temperature"], test.attrs["temperature_sensor_valid"]).all()
        assert np.isclose(base.attrs["Instrument_compass"], test.attrs["compass_sensor_valid"]).all()
        assert np.isclose(base.attrs["Instrument_tilt"], test.attrs["tilt_sensor_valid"]).all()
        ...
        def test_field(base, test, base_field_name, test_field_name):
            assert np.isclose(base[base_field_name].data, test[test_field_name].data).all()
        ...
        assert np.isclose(base["Heading"].data, test["heading"].data).all()
        assert np.isclose(base["Pitch"].data, test["pitch"].data).all()
        assert np.isclose(base["Roll"].data, test["roll"].data).all()
        ...
        for beam in test["beam"]:
            for range_bin in range(15):
                assert np.isclose(base[f"Vel_Beam{beam.data[()]}"].isel(AvgVelocityBeam_Range=range_bin).data, test["velocity_average"].sel(beam=beam.data[()]).isel(range_bin_average=range_bin).data).all()
                assert np.isclose(base[f"Cor_Beam{beam.data[()]}"].isel(AvgCorrelationBeam_Range=range_bin).data, test["correlation_average"].sel(beam=beam.data[()]).isel(range_bin_average=range_bin).data).all()
                assert np.isclose(base[f"Amp_Beam{beam.data[()]}"].isel(AvgAmplitudeBeam_Range=range_bin).data, test["amplitude_average"].sel(beam=beam.data[()]).isel(range_bin_average=range_bin).data).all()
        test.close()
