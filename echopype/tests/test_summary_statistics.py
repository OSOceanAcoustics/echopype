...
import numpy as np
import pandas as pd
from echopype.metrics.summary_statistics import delta_z,
							 					convert_to_linear,
												abundance,
												center_of_mass,
							 					inertia,
...
def test_abundance():
    ...
    assert np.allclose(abundance(ab_ds1), ab_ds1_SOL, rtol = 1e-09), 'Calculated output does not match expected output'
...
def test_center_of_mass():
    ...
    assert np.allclose(center_of_mass(cm_ds1), cm_ds1_SOL, rtol = 1e-09), 'Calculated output does not match expected output'
...
def test_inertia():
    ...
    assert np.allclose(inertia(in_ds1), in_ds1_SOL, rtol = 1e-09), 'Calculated output does not match expected output'
...
def test_evenness():
    ...
    assert np.allclose(evenness(ev_ds1), ev_ds1_SOL, rtol = 1e-09), 'Calculated output does not match expected output'
...
def test_aggregation():
    ...
    assert np.allclose(aggregation(ag_ds1), ag_ds1_SOL, rtol = 1e-09), 'Calculated output does not match expected output'
